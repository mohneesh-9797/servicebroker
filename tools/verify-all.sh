...
echo Verify OpenAPI
docker run --rm -v "${REPODIR}":/local abhisheknishant/generatornew_arm:v4.2.3 validate -i /local/openapi.yaml || rc=1

echo Verify Swagger
docker run --rm -v "${REPODIR}":/local abhisheknishant/generatornew_arm:v4.2.3 validate -i /local/swagger.yaml || rc=1
